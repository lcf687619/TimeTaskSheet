...
import com.mission.schedule.adapter.ComeSchAdapter;
...
public class ComeFriendAllSchFragment extends Fragment implements
		OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		return inflater.inflate(R.layout.fragment_importantallsch, container,
				false);
	}
	...
	private void init() {
		...
		important_listview = (SwipeXListView) view
		...
		headView = LayoutInflater.from(getActivity()).inflate(
				R.layout.fragment_allday_head, null);
		headView.setPadding(0, Utils.dipTopx(context, 20), 0, 0);
		up_rl = (RelativeLayout) headView.findViewById(R.id.up_rl);
		important_listview.setUpDown(up_rl);
		important_listview.addHeaderView(headView);
		...
		all_tv = (TextView) headView.findViewById(R.id.all_tv);
		outtwoday_tv = (TextView) headView.findViewById(R.id.outtwoday_tv);
		all_tv.setOnClickListener(this);
		outtwoday_tv.setOnClickListener(this);
		/**
		 * 获取屏幕的高度和宽度
		 */
		...
		try {
			allList = application.queryAllSchData(37, 0, 0);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (allList != null && allList.size() > 0) {
			all_tv.setVisibility(View.VISIBLE);
			outtwoday_tv.setVisibility(View.GONE);
		} else {
	...
	private void loadData() {
		try {
			...
			if (all_tv.getVisibility() == View.VISIBLE) {

			} else {
				otherlist = application.queryAllSchData(38, 0, 0);
			}
			yestodaylist = application.queryAllSchData(39, 0, 0);
			todaylist = application.queryAllSchData(40, 0, 0);
			tomorrowlist = application.queryAllSchData(41, 0, 0);
			inweeklist = application.queryAllSchData(42, 0, 0);
			outweeklist = application.queryAllSchData(43, 0, 0);
	...
	private void setAdapter() {
		adapter = new ComeSchAdapter(context, mList,
	...
	public void onClick(View v) {
		...
		case R.id.outtwoday_tv:
			...
			break;
		case R.id.all_tv:
			outtwoday_tv.setVisibility(View.VISIBLE);
			all_tv.setVisibility(View.GONE);
			loadData();
			adapter.notifyDataSetChanged();
			break;
