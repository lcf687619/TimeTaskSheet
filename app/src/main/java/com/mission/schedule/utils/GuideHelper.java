...
public class GuideHelper {
	...
	public GuideHelper show() {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			WindowManager.LayoutParams localLayoutParams = baseDialog
					.getWindow().getAttributes();
			localLayoutParams.flags = (WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS | localLayoutParams.flags);
		}

		baseDialog.setContentView(layout);
		baseDialog.getWindow().setLayout(
	...
	private void showIm(final FrameLayout layout, TipData... tipDatas) {
		...
		for (TipData data : tipDatas) {
			...
			for (View view : views) {
				...
				if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
					location[1] -= DisplayUtils.getStatusBarHeight(activity);
				}
				...
				if (data.needDrawView) {
					...
					if (bitmap != null) {
						...
						bitmap = Bitmap.createBitmap(vWidth, vHeight,
								Config.ARGB_8888);
				...
				if (rect == null) {
					rect = new Rect(location[0], location[1], location[0]
							+ vWidth, location[1] + vHeight);
				} else {
