...
import com.mission.schedule.swipexlistview.SwipeXListView.onRightViewWidthListener;
...
public class MyScheduleAdapter extends CommonAdapter<Map<String, String>>
	...
	public MyScheduleAdapter(Context context, List<Map<String, String>> lDatas,
			int layoutItemID, Handler handler, SwipeXListView swipeXlistview,
	...
	public void getViewItem(ViewHolder holder, final Map<String, String> mMap,
		...
		final LinearLayout outweek_ll = holder.getView(R.id.outweek_ll);// 一周以后布局
		final TextView outweekcount_tv = holder.getView(R.id.outweekcount_tv);
		...
		timeall_ll.setOnClickListener(new OnClickListener() {
			// 点击日历进入
			@Override
			public void onClick(View v) {
				setOnclick(position, mMap, 4);
			}
		});
		...
		outweek_ll.setOnClickListener(new OnClickListener() {
			// 点击一周后
			@Override
			public void onClick(View v) {
				setOnclick(position, mMap, 5);
				sharedPrefUtil.putString(context, ShareFile.USERFILE,
						ShareFile.OUTWEEKFAG, "1");
				outweekfag = "1";
				outweek_ll.setVisibility(View.GONE);
				outweekcount_tv.setVisibility(View.GONE);
			}
		});
		...
		if (position == 0) {// 当position=0，判断今天中是否有待办日程，有就隐藏标题栏，隐藏顺和天后，如果今天没有日程，然后判断明天和之后的
				...
				if ("0".equals(displayAlarm) && "1".equals(postpone)) {
					...
					if (!"".equals(localpath)) {
						if ("0".equals(localpath)) {
							timeall_tv_week.setTextColor(Color
									.parseColor("#7F7F7F"));
							timeall_tv_nongli.setTextColor(Color
									.parseColor("#7F7F7F"));
						} else {
						...
						timeall_tv_week.setTextColor(context.getResources()
								.getColor(R.color.white));
						timeall_tv_nongli.setTextColor(context.getResources()
								.getColor(R.color.white));
					}
					if (Math.abs(hour) >= 1) {
			...
			// } else {
			// timeall_after.setText(Math.abs(hour) + "小时后");
			//
			// }
			// if ("0".equals(displayAlarm)) {
			// if ("1".equals(postpone)) {
			...
			if (today.substring(0, 10).equals(
				...
				// if (!"".equals(localpath)) {
				// if ("0".equals(localpath)) {
				// bottom_week.setTextColor(Color
				// .parseColor("#7F7F7F"));
				// } else {
				// if ("1".equals(isEnd)) {
				// bottom_week.setTextColor(Color
		...
		if (uncount == 0 && todaycount == 0 && tomorrowcount == 0
			...
			if (position == uncount + todaycount + tomorrowcount + inweekcount
					- 1) {
				if ("1".equals(outweekfag)) {
					...
					if (outweekcount == 0) {
						...
						outweek_ll.setVisibility(View.VISIBLE);
						outweekcount_tv.setVisibility(View.VISIBLE);
						outweekcount_tv.setText("一周后(" + outweekcount + ")");
					}
				}
			} else {
				if ("1".equals(outweekfag)) {
