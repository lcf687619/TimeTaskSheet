...
public class NewMyFoundWoDeFengXiangAdapter extends
		CommonAdapter<NewMyFoundShouChangListBeen> {
	private static int sOneType = 0;
	private static int sTwoTYpe = 1;
	private Handler handler;
	private DisplayImageOptions options; // DisplayImageOptions是用于设置图片显示的类
	private ImageLoader imageLoader;
	private ImageLoadingListener animateFirstListener = new AnimateFirstDisplayListener();

	public NewMyFoundWoDeFengXiangAdapter(Context context,
	...
	public void getViewItem(ViewHolder holder,
		...
		TextView title = holder
				.getView(R.id.new_adapter_myfound_wodefengxiang_list_item_title);
		TextView content = holder
				.getView(R.id.new_adapter_myfound_wodefengxiang_list_item_count);
		TextView dianjinum = holder
				.getView(R.id.new_adapter_myfound_wodefengxiang_list_item_dianjiliang);
		TextView zhungtai = holder
				.getView(R.id.new_adapter_myfound_wodefengxiang_list_item_zhuangtai);
		CircularImage img = (CircularImage) holder
				.getView(R.id.new_adapter_myfound_wodefengxiang_list_item_img);
		...
		if ("".equals(StringUtils.getIsStringEqulesNull(item.content))) {
			...
			if (item.displaytime == 0) {
				if (!"".equals(StringUtils.getIsStringEqulesNull(item.date))) {
					contenttext = item.date.substring(5) + " " + "全天" + " "
							+ item.content;
				} else if ("".equals(StringUtils
						.getIsStringEqulesNull(item.date))) {
					contenttext = "今天" + " " + "全天" + " " + item.content;
				} else {
					if ("".equals(StringUtils
							.getIsStringEqulesNull(item.content))) {
						contenttext = "暂无提醒内容";
					} else {
				...
				if ("".equals(StringUtils.getIsStringEqulesNull(item.date))
						&& !"".equals(StringUtils
								.getIsStringEqulesNull(item.time))) {
					contenttext = "今天" + " " + item.time + " " + item.content;
				} else if (!"".equals(StringUtils
						.getIsStringEqulesNull(item.date))) {
					contenttext = item.date.substring(5) + " " + item.time + " "
							+ item.content;
				} else {
					if ("".equals(StringUtils
							.getIsStringEqulesNull(item.content))) {
