...
public class UpdateSchTiXingAdapter extends CommonAdapter<Map<String,String>> implements
	...
	public UpdateSchTiXingAdapter(Context context,
			List<Map<String, String>> lDatas, int layoutItemID,Handler handler,
			SwipeXListViewNoHead swipeXlistview, int width) {
	...
	public void getViewItem(ViewHolder holder, final Map<String, String> mMap,
		...
		if ("0".equals(mMap.get(ScheduleTable.schColorType))) {
			personstate_tv.setVisibility(View.GONE);
		} else {
			if (!"".equals(mMap.get(ScheduleTable.schColorType))) {
				String tagname = app.QueryTagNameData(
						Integer.parseInt(mMap.get(ScheduleTable.schColorType)))
						.get(CLCategoryTable.ctgText);
				if ("未分类".equals(tagname)) {
					personstate_tv.setVisibility(View.GONE);
				} else {
		...
		if (!"0".equals(mMap.get(ScheduleTable.schRepeatID))) {
			...
			if (!"".equals(StringUtils.getIsStringEqulesNull(mMap.get(ScheduleTable.schcRecommendName)))){
				comename_tv.setVisibility(View.VISIBLE);
				...
				comename_tv.setTextColor(context.getResources().getColor(
						R.color.mingtian_color));
				// timeall_rela_right.setBackground(context.getResources()
				// .getDrawable(R.drawable.bg_sch_come));
				if ("1".equals(mMap.get(ScheduleTable.schRead))) {
					timeall_rela_right.setBackground(context.getResources()
							.getDrawable(R.drawable.bg_sch_normal));
				} else if ("1".equals(important)) {
		...
		if ("3".equals(mMap.get(ScheduleTable.schIsEnd))
		...
		} else if ("0".equals(mMap.get(ScheduleTable.schIsEnd))
				&& "1".equals(mMap.get(ScheduleTable.schRead))) {
			newcome_ll.setVisibility(View.VISIBLE);
			newcome_tv.setVisibility(View.VISIBLE);
