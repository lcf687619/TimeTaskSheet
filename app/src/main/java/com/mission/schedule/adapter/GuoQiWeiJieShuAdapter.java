...
public class GuoQiWeiJieShuAdapter extends CommonAdapter<Map<String, String>>
	...
	public GuoQiWeiJieShuAdapter(Context context,
	...
	public void getViewItem(ViewHolder holder, final Map<String, String> mMap,
		...
		yestoday = DateUtil.formatDateTime(calendar.getTime());
		...
		if (position == 0) {// 当position=0，判断今天中是否有待办日程，有就隐藏标题栏，隐藏顺和天后，如果今天没有日程，然后判断明天和之后的
			...
			if (yestoday.substring(0, 10).equals(key)) {// &&
				...
				if ("0".equals(mMap.get(ScheduleTable.schDisplayTime))) {
			...
			if (yestoday.substring(0, 10).equals(
			...
			if (yestoday.substring(0, 10).toString().equals(key)) {// 昨天
				timeall_tv_lastday.setVisibility(View.GONE);
				timeall_after.setVisibility(View.GONE);
				bottom_month.setVisibility(View.GONE);
				bottom_week.setVisibility(View.GONE);
		...
		if (timeall_ll.getVisibility() == View.VISIBLE) {
			...
			params2.setMargins(Utils.dipTopx(context, 8),
					Utils.dipTopx(context, 20), Utils.dipTopx(context, 8), 0);
