...
public class NewFocusMobleTwoRichengAdapter extends CommonAdapter<FriendsRiChengBean> {
	...
	public NewFocusMobleTwoRichengAdapter(Context context,
			List<FriendsRiChengBean> lDatas, int layoutItemID, Handler handler,int mScreenWidth) {
...
//		ImageView guoqi_img = (ImageView) holder.getView(R.id.guoqi_img);
		RelativeLayout bgimglayout= (RelativeLayout)holder.getView(R.id.bg_img_moble_two_all);
		RelativeLayout bgimg= (RelativeLayout)holder.getView(R.id.bg_img_moble_two);
		if(item.imgPath==null||item.imgPath.equals("")){
			...
			ImageView img = (ImageView) holder.getView(R.id.bg_imgview_moble_two);
			String runableimgurl =URLConstants.图片 +imgurlend.replace(
					"\\/", "") + "&imageType=14&imageSizeType=1" ;
			imageLoader.displayImage(runableimgurl, img, options,animateFirstListener);
		}
...
//				.get("lunarCalendar");
		...
		String colorState2 = ""
				+ context.getResources().getColor(R.color.focus_moduletwo_content);
		String state1="";
		String state2="";		
