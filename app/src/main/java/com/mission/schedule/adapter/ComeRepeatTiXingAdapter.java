...
public class ComeRepeatTiXingAdapter extends
		RepeateCommonAdapter<Map<String, String>> {

	private Handler handler;

	public ComeRepeatTiXingAdapter(Context context,
			List<Map<String, String>> lDatas, int layoutItemID,
			Handler handler, SwipeXListViewNoHead listViewNoHead) {
		super(context, lDatas, R.layout.adapter_comerepeatetixing, handler,
				listViewNoHead);
	...
	@SuppressLint("NewApi")
	@Override
	public void getViewItem(final int position,
			List<Map<String, String>> mList, ViewHolder holder,
			final Map<String, String> map, Handler handler,
			SwipeXListViewNoHead listViewNoHead) {
		...
		TextView pasue_tv = holder.getView(R.id.pasue_tv);// 显示暂停
		LinearLayout ll = holder.getView(R.id.ll);// 整体布局
		...
		} else if (map.get(CLRepeatTable.repType).equals(
				mList.get(position - 1).get(CLRepeatTable.repType))) {
			date_ll.setVisibility(View.GONE);
		} else if (map.get(CLRepeatTable.repType).equals("4")
				&& mList.get(position - 1).get(CLRepeatTable.repType)
						.equals("6")) {
			date_ll.setVisibility(View.GONE);
		} else if (map.get(CLRepeatTable.repType).equals("6")
				&& mList.get(position - 1).get(CLRepeatTable.repType)
						.equals("4")) {
			date_ll.setVisibility(View.GONE);
		} else {
		...
		if (!"".equals(map.get(CLRepeatTable.repcommendedUserName))
				&& !"null".equals(map.get(CLRepeatTable.repcommendedUserName))) {
			comename_tv.setVisibility(View.VISIBLE);
			comename_tv.setText("来自" + " "
					+ map.get(CLRepeatTable.repcommendedUserName));
