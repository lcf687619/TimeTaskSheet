...
public class EditNewFocusShareRepeatActivity extends BaseActivity implements
		OnClickListener {
	...
	String imagepath = "";
	App app = null;
	NewFocusShareBean shareBean = null;
	...
	protected void setContentView() {
		setContentView(R.layout.activity_addnewfocussharerepeat);
	}
	...
	protected void init(Bundle savedInstanceState) {
		...
		shareBean = (NewFocusShareBean) getIntent()
				.getSerializableExtra("bean");
		url = StringUtils.getIsStringEqulesNull(shareBean.fstWebUR);
		imagepath = StringUtils.getIsStringEqulesNull(shareBean.fstImagePath);
		datetype = shareBean.fstRepType;
		dateselect = shareBean.fstParameter.replace("[", "").replace("]", "");
		isAlarm = shareBean.fstIsAlarm;
		setBefore = shareBean.fstBeforeTime;
		...
		if ("0".equals(shareBean.fstDisplayTime)) {
			time_tv.setText("全天");
		} else {
			time_tv.setText(shareBean.fstTime);
		}
		...
		date_tv.setText(dateStr);
		morenlingshen_tv.setText(shareBean.fstRingDesc);
		ringcode = shareBean.fstRingCode;
		addrepeatcontent_et.setText(shareBean.fstContent);
		addrepeatcontent_et.setSelection(addrepeatcontent_et.getText()
				.toString().length());
		if (!"".equals(url) && !"".equals(imagepath)) {
			choosecontent_tv.setText("附加信息     URL+图片");
		} else if(!"".equals(url) && "".equals(imagepath)){
			choosecontent_tv.setText("附加信息     URL");
		} else if("".equals(url) && !"".equals(imagepath)){
			choosecontent_tv.setText("附加信息     图片");
		} else{
	...
	public void onClick(View v) {
		...
		case R.id.choosecontent_tv:
			intent = new Intent(context, AddSchWebUrlAndImageActivity.class);
		...
		case R.id.top_ll_right:
			...
			if (addrepeatcontent_et.getText().toString().trim() != null
					&& !"".equals(addrepeatcontent_et.getText().toString()
							.trim())) {
				boolean flag = false;
				...
				if (dateIndex == 1) {
					...
					paramter = "[" + "\"" + dateselect_tv.getText().toString()
							+ "\"" + "]";
					if ("0".equals(yearType)) {
						parstr = "";
					} else {
						parstr = changeValue.changaSZ(dateselect_tv.getText()
								.toString());
					}
				}
				if (shareBean.fstRepeatId < 0) {
					app.deleteNewFocusShareData(1,
							NewFocusShareEditActivity.been.id, 0,
							shareBean.fstRepeatId, "");
					flag = app.insertNewFocusFromIntenetData(
							NewFocusShareEditActivity.been.id,
							shareBean.fstSchID, 1, before, isAlarm, timeState,
							shareBean.fstColorType, shareBean.fstIsPostpone,
							shareBean.fstIsImportant, shareBean.fstIsEnd,
							shareBean.fstSourceType, shareBean.fstRepeatId,
							shareBean.fstOpenState, shareBean.fstRepeatLink,
							shareBean.fstRecommendId, shareBean.fstIsRead,
							shareBean.fstAID, shareBean.fstIsPuase,
							shareBean.fstRepStateOne, shareBean.fstRepStateTwo,
							shareBean.fstRepInStable, shareBean.fstPostState,
							dateIndex, urlstate, 1, paramter,
							addrepeatcontent_et.getText().toString(),
							shareBean.fstDate, alltime, ringcode, ringdesc,
							shareBean.fstTags, shareBean.fstSourceDesc,
							shareBean.fstSourceDescSpare,
							shareBean.fstRepeatDate, shareBean.fstRepStartDate,
							bean.repNextCreatedTime, bean.repLastCreatedTime,
							DateUtil.formatDateTimeSs(new Date()), "", "",
							shareBean.fstRecommendName, url, imagepath, parstr,
							DateUtil.formatDateTime(new Date()),
							DateUtil.formatDateTime(new Date()), "", "", "",
							"", "");
					// flag = app.updateNewFocusFromRepeatData(shareBean.fstID,
					// NewFocusShareEditActivity.been.id,
					// shareBean.fstSchID, 1, before, isAlarm, timeState,
					// shareBean.fstColorType, shareBean.fstIsPostpone,
					// shareBean.fstIsImportant, shareBean.fstIsEnd,
					...
					if (flag) {
						...
						app.insertNewFocusData(
								NewFocusShareEditActivity.been.id, 0, before,
								isAlarm, timeState, shareBean.fstColorType,
								shareBean.fstIsPostpone,
								shareBean.fstIsImportant, shareBean.fstIsEnd,
								shareBean.fstSourceType, shareBean.fstRepeatId,
								shareBean.fstOpenState,
								shareBean.fstRepeatLink,
								shareBean.fstRecommendId, shareBean.fstIsRead,
								shareBean.fstAID, shareBean.fstIsPuase,
								shareBean.fstRepStateOne,
								shareBean.fstRepStateTwo,
								shareBean.fstRepInStable,
								shareBean.fstPostState, 0, urlstate, 0,
								paramter, addrepeatcontent_et.getText()
										.toString(), shareBean.fstDate,
								alltime, ringcode, ringdesc, shareBean.fstTags,
								shareBean.fstSourceDesc,
								shareBean.fstSourceDescSpare,
								shareBean.fstRepeatDate,
								shareBean.fstRepStartDate,
								bean.repNextCreatedTime,
								bean.repLastCreatedTime, DateUtil
										.formatDateTimeSs(new Date()), "", "",
								shareBean.fstRecommendName, url, imagepath,
								parstr, DateUtil.formatDateTime(new Date()),
								DateUtil.formatDateTime(new Date()), "", "",
								"", "", "");
					}
				} else {
	...
	class MyClick implements View.OnClickListener {
		...
		private MyClick(Dialog dialog, View more_pop_menu) {
			...
			tv_clock_state.setVisibility(View.VISIBLE);
