...
public class UpdateSchTiXingActivity extends BaseActivity implements
	...
	// List<Map<String, String>> otherlist = new ArrayList<Map<String,
	// String>>();
	// List<Map<String, String>> yestodaylist = new ArrayList<Map<String,
	// String>>();
	...
	private void setReadState() {
		for (int i = 0; i < mList.size(); i++) {
			if (!"0".equals(mList.get(i).get(ScheduleTable.schAID))) {
				app.updateReadState(
						Integer.parseInt(mList.get(i).get(ScheduleTable.schID)),
						0);
			} else {
				if ("2".equals(mList.get(i).get(ScheduleTable.schIsEnd))) {
					app.updateReadState(
							Integer.parseInt(mList.get(i).get(
									ScheduleTable.schID)), 3);
				} else {
					app.updateReadState1(Integer.parseInt(mList.get(i).get(
							ScheduleTable.schID)));
				}
			}
		}
	...
	private void loadData() {
		try {
			...
			// otherlist = application.queryComeData(1);
			// yestodaylist = application.queryComeData(2);
			todaylist = application.queryAllSchData(12,0, 0);
			tomorrowlist = application.queryAllSchData(13,0, 0);
			inweeklist = application.queryAllSchData(14,0, 0);
			outweeklist = application.queryAllSchData(15,0, 0);
	...
	protected void setAdapter() {
		adapter = new UpdateSchTiXingAdapter(context, mList,R.layout.adapter_updateschtixing, handler,
				important_listview, mScreenWidth);
		important_listview.setPullLoadEnable(false);
		...
		public MyGeneralDetailOnClick(Dialog dialog, Map<String, String> mMap,
			...
			if (today.equals(key)) {// 今天
				...
				if (DateUtil.parseDateTime(DateUtil.formatDateTime(new Date()))
						.after(DateUtil.parseDateTime(DateUtil
								.formatDateTime(dateStr)))) {
					timebefore_tv.setText(Math.abs(day) + 1 + "天前");
				} else {
		...
		public void onClick(View v) {
			...
			case R.id.more_setdaiban:// 设为待办 今天+全天+顺延
				try {
					...
					adapter.notifyDataSetChanged();
					// important_listview
					// .setSelectionFromTop(scrolledX, scrolledY);
					RefreshMyData();
					QueryAlarmData.writeAlarm(getApplicationContext());
				} catch (Exception e) {
					e.printStackTrace();
			...
			case R.id.more_shitingmusic:// 试听铃声
				...
				try {
					AssetFileDescriptor fileDescriptor = getAssets().openFd(
							mMap.get(ScheduleTable.schRingCode) + ".mp3");
			...
			case R.id.after_autopostone:
				...
				QueryAlarmData.writeAlarm(getApplicationContext());
				adapter.notifyDataSetChanged();
				// important_listview.setSelectionFromTop(scrolledX,
				// scrolledY);
				RefreshMyData();
				dialog.dismiss();
				break;
			case R.id.after_onehour:
				try {
					date = inWeekUtils.AfterOneHours(key,
							mMap.get(ScheduleTable.schTime));
					app.updateScheduleDateData(id, date.subSequence(0, 10)
