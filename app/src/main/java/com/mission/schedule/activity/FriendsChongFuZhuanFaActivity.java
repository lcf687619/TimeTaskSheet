...
public class FriendsChongFuZhuanFaActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void init(Bundle savedInstanceState) {
		...
		userId = prefUtil.getString(context, ShareFile.USERFILE,
				ShareFile.USERID, "");
		name = prefUtil.getString(context, ShareFile.USERFILE,
				ShareFile.USERNAME, "");
		chongFuBean = (FriendsChongFuBean) getIntent().getSerializableExtra(
				"bean");
		LoadData();
	}
	...
	private void item() {
		friends_lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				pairs.put("tbuserFrendsMessage.messge", chongFuBean.repContent);
				pairs.put("tbuserFrendsMessage.status",
						String.valueOf(chongFuBean.repChangeState));
				pairs.put("tbuserFrendsMessage.cIsAlarm",
						String.valueOf(chongFuBean.repIsAlarm));
				pairs.put("tbuserFrendsMessage.cdate", "");
				pairs.put("tbuserFrendsMessage.ctime", chongFuBean.repTime);
				pairs.put("tbuserFrendsMessage.cRecommendName", name);
				pairs.put("tbuserFrendsMessage.cAlarmSound",
						chongFuBean.repRingCode);
				pairs.put("tbuserFrendsMessage.cAlarmSoundDesc",
						chongFuBean.repRingDesc);
				pairs.put("tbuserFrendsMessage.repType",
						String.valueOf(chongFuBean.repType));// 1每天,2每周,3每月,4每年,5工作日
				/**
				 * 生成规则 1每天[] 2 每周[“1”] 3每月[“1”]每月1号 4每年[“01-01”]每年1月1号 5工作日[]
				 */
				pairs.put("tbuserFrendsMessage.repTypeParameter",
						chongFuBean.repTypeParameter);
				pairs.put("tbuserFrendsMessage.cPostpone", String.valueOf(0));// 是否顺延(0否,1是)',
				pairs.put("tbuserFrendsMessage.cTags", chongFuBean.cTags);// 分类标签'
				pairs.put("tbuserFrendsMessage.cType",
						String.valueOf(chongFuBean.repSourceType));// 记事类别(0普通的,1带url的,2备忘录以上的都需要带公用参数)',
				pairs.put("tbuserFrendsMessage.cTypeDesc",
						chongFuBean.repSourceDesc);// 当记事类别为1时所带的url链接',
				pairs.put("tbuserFrendsMessage.cTypeSpare",
						chongFuBean.repSourceDescSpare);// 当记事类别为1时所带的url链接描述',
				pairs.put("tbuserFrendsMessage.cOpenstate",
						String.valueOf(chongFuBean.repOpenState));// 公开状态(0否,1是,2仅好友可见)',
				pairs.put("tbuserFrendsMessage.cLightAppId", "");// 轻应用与记事绑定的唯一ID'
				pairs.put("tbuserFrendsMessage.repcolortype",
						String.valueOf(chongFuBean.repColorType));// 记事颜色类别',
				pairs.put("tbuserFrendsMessage.repstartdate",
						chongFuBean.repStartDate);// 下一次重复闹钟起始时间'
				pairs.put("tbuserFrendsMessage.repnextcreatedtime",
						chongFuBean.repNextCreatedTime);// 下一次生成日期 如 2012-01-01
														// 12：00
				pairs.put("tbuserFrendsMessage.replastcreatedtime",
						chongFuBean.repLastCreatedTime);// 之前最后一次生成时间
														// 如2012-01-01 12：00
				/**
				 * 是否显示时间 0否 1是 默认为1
				 */
				pairs.put("tbuserFrendsMessage.repdisplaytime",
						String.valueOf(chongFuBean.repDisplayTime));
				pairs.put("tbuserFrendsMessage.repinitialcreatedtime",
						chongFuBean.repInitialCreatedTime);// 初始创建时间
				pairs.put("tbuserFrendsMessage.aType", chongFuBean.aType + "");// 0无附加信息1附加连接2附加图片3连接和图片
				pairs.put("tbuserFrendsMessage.webUrl", chongFuBean.webUrl);
				pairs.put("tbuserFrendsMessage.imgPath", chongFuBean.imgPath);
				pairs.put("tbuserFrendsMessage.repInSTable",
						chongFuBean.repInSTable + "");

				app.insertMFMessageSendData(Integer.parseInt(userId),
						friendsBean.fId, chongFuBean.repIsAlarm,
						chongFuBean.repOpenState, 0, chongFuBean.repColorType,
						chongFuBean.repDisplayTime, chongFuBean.repBeforeTime,
						chongFuBean.repSourceType, chongFuBean.repType,
						chongFuBean.repTypeParameter, chongFuBean.repContent,
						chongFuBean.repCreateTime, "", chongFuBean.repTime,
						chongFuBean.repSourceDesc,
						chongFuBean.repSourceDescSpare, chongFuBean.cTags,
						chongFuBean.repRingDesc, chongFuBean.repRingCode,
						chongFuBean.repStartDate,
						chongFuBean.repInitialCreatedTime,
						chongFuBean.repLastCreatedTime,
						chongFuBean.repNextCreatedTime, 2, chongFuBean.aType,
						chongFuBean.webUrl, chongFuBean.imgPath,
						chongFuBean.repInSTable);
	...
	private void alertDialog() {
		...
		delete_ok.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				builder.cancel();
				FriendsChongFuZhuanFaActivity.this.finish();
			}
		});
