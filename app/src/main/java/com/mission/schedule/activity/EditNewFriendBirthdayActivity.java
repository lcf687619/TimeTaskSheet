...
public class EditNewFriendBirthdayActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void setContentView() {
		setContentView(R.layout.activity_newfriendbirthday);
	}
			...
			if (addrepeatcontent_et.getText().toString().trim() != null
				...
				try {
					...
					if ("每天".equals(date_tv.getText().toString())) {
						...
						flag = app
								.insertMessageSendData(userid, friendid, 0, 0,
										2, isAlarm, 0, coclor, timeState,
										before, 0, dateIndex, atype, 0, 0, 0,
										0, "[]", addrepeatcontent_et.getText()
												.toString().trim(),
						...
						if (flag) {
							app.insertMessageSendData(userid, friendid, 0, 0,
									1, isAlarm, 0, coclor, timeState, before,
									0, 0, atype, 0, 0, 0, 0, "[]",
						...
						flag = app
								.insertMessageSendData(userid, friendid, 0, 0,
										2, isAlarm, 0, coclor, timeState,
										before, 0, dateIndex, atype, 0, 0, 0,
										0, str, addrepeatcontent_et.getText()
												.toString().trim(),
										DateUtil.formatDate(new Date()),
										alltime, "", "", "", morenlingshen_tv
												.getText().toString().trim(),
						...
						String str = "[" + "\"" + dateselect_tv.getText().toString() + "\""
								+ "]";
						...
						if ("0".equals(yearType)) {
							flag = app.insertMessageSendData(userid, friendid,
									0, 0, 2, isAlarm, 0, coclor, timeState,
									before, 0, dateIndex, atype, 0, 0, 0, 0,
									str, addrepeatcontent_et.getText()
											.toString().trim(),
							...
							String parstr = changeValue
									.changaSZ(dateselect_tv.getText()
											.toString());
							flag = app.insertMessageSendData(userid, friendid,
									0, 0, 2, isAlarm, 0, coclor, timeState,
									before, 0, 6, atype, 0, 0, 0, 0, str,
									addrepeatcontent_et.getText().toString()
											.trim(), DateUtil
