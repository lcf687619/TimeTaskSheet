...
public class EditBirthRepeatActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void init(Bundle savedInstanceState) {
		...
		yearType = getIntent().getStringExtra("yeartype");
		content = getIntent().getStringExtra("content");
		repdate = getIntent().getStringExtra("date");
		beforetime = getIntent().getStringExtra("beforetime");
		ringcode = getIntent().getStringExtra("alamsound");
		ring = getIntent().getStringExtra("alamsoundDesc");
		tagname = getIntent().getStringExtra("tagname");
		coclor = getIntent().getIntExtra("coclor", 0);
		isAlarm = 3;
		dateStr = "每年";
		dateselect_tv.setText(repdate);
		...
		if ("0".equals(beforetime)) {
			...
			beforeStr = "2小时";
		} else if ("1440".equals(beforetime)) {
			beforeStr = "1天";
		} else if ("2880".equals(beforetime)) {
			beforeStr = "2天";
		} else if ("241920".equals(beforetime)) {
			beforeStr = "1周";
		}
		if ("0".equals(beforetime)) {
		...
		time_tv.setText("08:20");
		morenlingshen_tv.setText(ring);
		addrepeatcontent_et.setText(content);
	...
	public void onClick(View v) {
		...
		case R.id.top_ll_right:
			...
			if ("全天".equals(time_tv.getText().toString())) {
				timeState = 0;
				alltime = prefUtil.getString(mContext, ShareFile.USERFILE,
						ShareFile.ALLTIME, "08:58");
			} else {
				timeState = 1;
			...
			if (addrepeatcontent_et.getText().toString().trim() != null
				...
				try {
					...
					if ("每天".equals(date_tv.getText().toString())) {
						...
						flag = app.insertCLRepeatTableData(before, coclor,
								timeState, dateIndex, isAlarm, 0, 0, 0, 1,
								"[]", bean.repNextCreatedTime,
								bean.repLastCreatedTime,
								DateUtil.formatDateTime(new Date()),
								bean.repNextCreatedTime, addrepeatcontent_et
										.getText().toString().trim(),
								DateUtil.formatDateTime(new Date()), "", "",
								alltime, morenlingshen_tv.getText().toString()
										.trim(), ringcode,
								DateUtil.formatDateTime(new Date()), 0, "", 0,
								"", "", 0, 0, urlstate, url, "",
								Integer.parseInt(repselectstate), 0, "", 0, 0);

						if ("0".equals(repselectstate)) {
							app.insertScheduleData(addrepeatcontent_et
									.getText().toString().trim(),
									bean.repNextCreatedTime.substring(0, 10),
									alltime, isAlarm, before, timeState, 0, 0,
									coclor, 0, bean.repNextCreatedTime, "", 0,
									"", "", app.repschId,
									bean.repNextCreatedTime,
									DateUtil.formatDateTime(new Date()), 0, 0,
									1, morenlingshen_tv.getText().toString()
											.trim(), ringcode, "", 0, 0,
									urlstate, url, "", 0, 0, 0);
						}
						...
						if (dateselect_tv.getText().toString().contains(",")) {
							dateselectData = dateselect_tv.getText().toString()
									.split(",");
						} else {
							dateselectData = new String[] { dateselect_tv
									.getText().toString() };
						}
						for (int i = 0; i < dateselectData.length; i++) {
							bean = RepeatDateUtils.saveCalendar(alltime, 2,
									dateselectData[i], "");
							String str;
							if (dateselectData[i].contains("星期")) {
								if ("一".equals(dateselectData[i].substring(2,
										dateselectData[i].length()))) {
									str = "[" + "\"" + 1 + "\"" + "]";
								} else if ("二".equals(dateselectData[i]
										.substring(2,
												dateselectData[i].length()))) {
									str = "[" + "\"" + 2 + "\"" + "]";
								} else if ("三".equals(dateselectData[i]
										.substring(2,
												dateselectData[i].length()))) {
									str = "[" + "\"" + 3 + "\"" + "]";
								} else if ("四".equals(dateselectData[i]
										.substring(2,
												dateselectData[i].length()))) {
									str = "[" + "\"" + 4 + "\"" + "]";
								} else if ("五".equals(dateselectData[i]
										.substring(2,
												dateselectData[i].length()))) {
									str = "[" + "\"" + 5 + "\"" + "]";
								} else if ("六".equals(dateselectData[i]
										.substring(2,
												dateselectData[i].length()))) {
									str = "[" + "\"" + 6 + "\"" + "]";
								} else {
							...
							flag = app.insertCLRepeatTableData(before, coclor,
									...
									DateUtil.formatDateTime(new Date()), 0, "",
									0, "", "", 0, 0, urlstate, url, "",
									Integer.parseInt(repselectstate), 0, "", 0,
									0);
							if ("0".equals(repselectstate)) {
								app.insertScheduleData(addrepeatcontent_et
										.getText().toString().trim(),
										...
										isAlarm, before, timeState, 0, 0,
										coclor, 0, bean.repNextCreatedTime, "",
										0, "", "", app.repschId,
										bean.repNextCreatedTime, DateUtil
												.formatDateTime(new Date()), 0,
										0, 1, morenlingshen_tv.getText()
												.toString().trim(), ringcode,
										"", 0, 0, urlstate, url, "", 0, 0, 0);
							}
						}
						...
						for (int i = 0; i < dateselectData.length; i++) {
							bean = RepeatDateUtils.saveCalendar(alltime, 3,
									dateselectData[i], "");
							String str;
							if (dateselectData[i].contains("日")) {
								str = "["
										+ "\""
										+ dateselectData[i].substring(0,
												dateselectData[i].length() - 1)
										+ "\"" + "]";
							} else {
								str = "[" + "\"" + dateselectData[i] + "\""
										+ "]";
							}
							flag = app.insertCLRepeatTableData(before, coclor,
									timeState, dateIndex, isAlarm, 0, 0, 0, 1,
						...
						if (dateselect_tv.getText().toString().contains("、")) {
							...
							for (int i = 0; i < dateselectData.length; i++) {
								...
								if ("0".equals(repselectstate)) {
									app.insertScheduleData(
											...
											bean.repNextCreatedTime,
											DateUtil.formatDateTime(new Date()),
											0, 0, 1, morenlingshen_tv.getText()
													.toString().trim(),
											ringcode, "", 0, 0, urlstate, url,
											"", 0, 0, 0);
								}
							}
						} else {
							...
							bean = RepeatDateUtils.saveCalendar(alltime, 4,
									...
									yearType);
							if ("0".equals(yearType)) {
								flag = app.insertCLRepeatTableData(before,
										coclor, timeState, dateIndex, isAlarm,
										0, 0, 1, 1, str,
										bean.repNextCreatedTime,
										bean.repLastCreatedTime,
										DateUtil.formatDateTime(new Date()),
								...
								flag = app.insertCLRepeatTableData(before,
										coclor, timeState, 6, isAlarm, 0, 0, 0,
										...
										"", 0, "", "", 0, 0, urlstate, url, "",
										Integer.parseInt(repselectstate), 0,
										changeValue.changaSZ(dateselect_tv
