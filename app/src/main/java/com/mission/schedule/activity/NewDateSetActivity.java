...
import com.mission.schedule.fragment.NewYangLiCalendarFragment;
import com.mission.schedule.fragment.NewYinLiCalendarFragment;
...
public class NewDateSetActivity extends BaseActivity implements OnClickListener{
	...
	protected void setContentView() {
		setContentView(R.layout.activity_newdateset);
	}
	...
	@SuppressLint("NewApi") private void init() {
		...
		if (which == 0) {
			...
			nongli_tv.setTextColor(getResources().getColor(R.color.mingtian_color));
			NewYangLiCalendarFragment solarCalendarFragment = (NewYangLiCalendarFragment) fragmentManager.findFragmentById(R.id.fragment_solar);
//			solarCalendarFragment.setMonthAndDay(monthStrT, Integer.parseInt(dayStr));
		} else if (which == 1) {
			...
			yangli_tv.setTextColor(getResources().getColor(R.color.mingtian_color));
			nongli_tv.setTextColor(getResources().getColor(R.color.white));
			NewYinLiCalendarFragment lunarFragment = (NewYinLiCalendarFragment) fragmentManager.findFragmentById(R.id.fragment_lunar);
...
//			lunarFragment.setMonthAndDay(1, 1);
	...
	public void onClick(View v) {
		...
		case R.id.dateset_cb_solar://阳历
			...
			if(NewYinLiCalendarFragment.strlist.size()>0){
				alertDeleteDialog(0);
			}else{
		...
		case R.id.dateset_cb_lunar://阴历
			...
			if(NewYangLiCalendarFragment.strlist.size()>0){
				alertDeleteDialog(1);
			} else {
		...
		case R.id.sure_ll:
			...
			if(which==0){
				...
				intent1.putStringArrayListExtra("list", (ArrayList<String>) NewYangLiCalendarFragment.strlist);
				setResult(Activity.RESULT_OK, intent1);
			}else{
				intent1.putExtra("type", "1");// 阴历
				intent1.putStringArrayListExtra("list", (ArrayList<String>) NewYinLiCalendarFragment.strlist);
				setResult(Activity.RESULT_OK, intent1);
			}
			finish();
			break;
	...
	private void alertDeleteDialog(final int type) {
		...
		delete_ok.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if(type==0){
					NewYinLiCalendarFragment.strlist.clear();
					setFragmentIndicator(0);
					EventBus.getDefault().post(new FristFragment("5"));
				} else {
					NewYangLiCalendarFragment.strlist.clear();
					setFragmentIndicator(1);
					EventBus.getDefault().post(new FristFragment("6"));
				}
				builder.cancel();
