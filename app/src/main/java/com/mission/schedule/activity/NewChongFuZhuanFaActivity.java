...
public class NewChongFuZhuanFaActivity extends BaseActivity implements
		OnClickListener {
		...
		friends_lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				boolean fag = app.insertMessageSendData(
						Integer.parseInt(userId), friendsBean.fId,
						bean.nfmCalendarId, bean.nfmOpenState, 2,
						bean.nfmIsAlarm, bean.nfmPostpone, bean.nfmColorType,
						bean.nfmDisplayTime, bean.nfmBeforeTime,
						bean.nfmSourceType, bean.nfmType, bean.nfmAType,
						bean.nfmInSTable, 0, 0, bean.nfmPostState,
						...
						bean.nfmIsPuase, bean.parReamrk,
						DateUtil.formatDateTimeSs(new Date()));
				ID = app.nfmId;
				if (fag) {
					app.insertMessageSendData(Integer.parseInt(userId),
							friendsBean.fId, bean.nfmCalendarId,
							bean.nfmOpenState, 1, bean.nfmIsAlarm,
							bean.nfmPostpone, bean.nfmColorType,
							bean.nfmDisplayTime, bean.nfmBeforeTime,
							bean.nfmSourceType, bean.nfmType, bean.nfmAType,
							bean.nfmInSTable, 0, 0, bean.nfmPostState,
							bean.nfmParameter, bean.nfmContent,
							bean.nfmNextCreatedTime.substring(0, 10),
							bean.nfmTime, bean.nfmSourceDesc,
							bean.nfmSourceDescSpare, bean.nfmTags,
							bean.nfmRingDesc, bean.nfmRingCode,
							bean.nfmStartDate, bean.nfmInitialCreatedTime,
							bean.nfmLastCreatedTime, bean.nfmNextCreatedTime,
							bean.nfmWebURL, bean.nfmImagePath, myName,
							bean.nfmRemark, 0, ID, 0, "", 0, 0, "", 0,
							"", DateUtil.formatDateTimeSs(new Date()));
				}
				UpdateLoadData();
			}
	...
	private void alertDialog() {
		...
		delete_ok.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				builder.cancel();
				NewChongFuZhuanFaActivity.this.finish();
			}
		});
