...
public class EditSettingNewFocusShareSmallTitleActivity extends BaseActivity implements OnClickListener{
	...
	@ViewResId(id = R.id.sharetitle_et)
	private EditText sharetitle_et;
	...
	protected void setContentView() {
		setContentView(R.layout.activity_editsettingnewfocussharesmalltitle);
	}
	...
	protected void init(Bundle savedInstanceState) {
		...
		middle_tv.setText("分享标题");
		sharetitle_et.setText(sharetitle);
		sharetitle_et.setSelection(sharetitle.length());		
	}
		...
		StringRequest request = new StringRequest(Method.POST, path, new Response.Listener<String>() {
			...
			public void onResponse(String result) {
				...
				if(!TextUtils.isEmpty(result)){
					...
					listBeen.remark5 = sharetitle_et.getText().toString();
					listBeen.remark6 = contentstr;
					Intent intent = new Intent();
					intent.putExtra("bean", listBeen);
					setResult(Activity.RESULT_OK, intent);
					EditSettingNewFocusShareSmallTitleActivity.this.finish();
				}
			}
			...
			protected Map<String, String> getParams() throws AuthFailureError {
				...
				map.put("tbAttentionUserSpace.remark5", sharetitle_et.getText().toString().trim());
				return map;
			}
		};
		request.setTag("up");
