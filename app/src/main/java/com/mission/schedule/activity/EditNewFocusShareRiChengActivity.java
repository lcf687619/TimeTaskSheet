...
public class EditNewFocusShareRiChengActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void init(Bundle savedInstanceState) {
		...
		UserId = sharedPrefUtil.getString(mcontext, ShareFile.USERFILE,
				ShareFile.USERID, "");
		setBefore = bean.fstBeforeTime;
		ringdesc = bean.fstRingDesc;
		url = StringUtils.getIsStringEqulesNull(bean.fstWebUR);
		imagepath = StringUtils.getIsStringEqulesNull(bean.fstImagePath);
		alltime = sharedPrefUtil.getString(mcontext, ShareFile.USERFILE,
				ShareFile.ALLTIME, "08:58");
		ringcode = bean.fstRingCode;
		timestr = bean.fstDisplayTime;
		rilibianhua_tv.setText(DateUtil.formatDate(DateUtil
				.parseDate(bean.fstDate)));
		newbuild_ymd_state.setText("今天  "
				+ CharacterUtil.getWeekOfDate(this,
						DateUtil.parseDate(bean.fstDate)));
		addeverytask_tv.setText(bean.fstContent);
		addeverytask_tv.setSelection(bean.fstContent.length());
		String beforeStr = "";
		if (setBefore == 0) {
			beforeStr = "准时提醒";
		} else if (setBefore == 5) {
	...
	public void onClick(View v) {
		...
		case R.id.top_ll_right:
			...
			if (bean.fstRepeatId != 0 && bean.fstRepeatLink == 1) {
				Map<String, String> map = new HashMap<String, String>();
				map.put(CLFindScheduleTable.fstRepType, bean.fstRepType + "");
				map.put(CLFindScheduleTable.fstTime, bean.fstTime);
				map.put(CLFindScheduleTable.fstParameter, bean.fstParameter);
				RepeatBean beandate = NewFocusShareEditActivity
						.getNextChildTime(map);
				if (beandate.repNextCreatedTime.equals(bean.fstRepeatDate)) {
					app.updateNewFocusShareRepeatData(
							NewFocusShareEditActivity.been.id,
							bean.fstRepeatId, bean.fstRepDateOne,
							bean.fstRepeatDate, bean.fstRepStateOne, 1);
				} else {
					app.updateNewFocusShareRepeatData(
							NewFocusShareEditActivity.been.id,
							bean.fstRepeatId, bean.fstRepeatDate,
							bean.fstRepDateTwo, 1, bean.fstRepStateTwo);
				}
				app.deleteNewFocusShareData(5,NewFocusShareEditActivity.been.id,bean.fstSchID,bean.fstRepeatId,"");
				app.insertNewFocusData(
						NewFocusShareEditActivity.been.id,0,
						before, isAlarm, displaytime, bean.fstColorType,
						bean.fstIsPostpone, bean.fstIsImportant, bean.fstIsEnd,
						bean.fstSourceType, 0, bean.fstOpenState, 0,
						bean.fstRecommendId, bean.fstIsRead, bean.fstAID,
						bean.fstIsPuase, bean.fstRepStateOne,
						bean.fstRepStateTwo, bean.fstRepInStable,
						bean.fstPostState, 0, urlstate, 1, bean.fstParameter,
						addeverytask_tv.getText().toString(), rilibianhua_tv
								.getText().toString(), time, ringcode,
						ringdesc, bean.fstTags, bean.fstSourceDesc,
						bean.fstSourceDescSpare, "", "",
						bean.fstRpNextCreatedTime, bean.fstRepLastCreatedTime,
						bean.fstRepInitialCreatedTime, bean.fstRepDateOne,
						bean.fstRepDateTwo, bean.fstRecommendName, url,
						imagepath, "", createTime, schUpdateTime,
						bean.fstReamrk1, bean.fstReamrk2, bean.fstReamrk3,
						bean.fstReamrk4, bean.fstReamrk5);
			} else {
				if (bean.fstSchID < 0) {
					app.updateNewFocusFromIntenetData(
							NewFocusShareEditActivity.been.id, bean.fstSchID,
							0, before, isAlarm, displaytime, bean.fstColorType,
							bean.fstIsPostpone, bean.fstIsImportant,
							bean.fstIsEnd, bean.fstSourceType, 0,
							bean.fstOpenState, 0, bean.fstRecommendId,
							bean.fstIsRead, bean.fstAID, bean.fstIsPuase,
							bean.fstRepStateOne, bean.fstRepStateTwo,
							bean.fstRepInStable, bean.fstPostState, 0,
							urlstate, 1, bean.fstParameter, addeverytask_tv
									.getText().toString(), rilibianhua_tv
									.getText().toString(), time, ringcode,
							ringdesc, bean.fstTags, bean.fstSourceDesc,
							bean.fstSourceDescSpare, "", "",
							bean.fstRpNextCreatedTime,
							bean.fstRepLastCreatedTime,
							bean.fstRepInitialCreatedTime, bean.fstRepDateOne,
							bean.fstRepDateTwo, bean.fstRecommendName, url,
							imagepath, "", createTime, schUpdateTime,
							bean.fstReamrk1, bean.fstReamrk2, bean.fstReamrk3,
					...
					app.updateNewFocusFromIntenetData(
							...
							bean.fstRepStateOne, bean.fstRepStateTwo,
							bean.fstRepInStable, bean.fstPostState, 0,
							bean.fstAType, 2, bean.fstParameter,
							addeverytask_tv.getText().toString(),
							rilibianhua_tv.getText().toString(), time,
							ringcode, ringdesc, bean.fstTags,
							bean.fstSourceDesc, bean.fstSourceDescSpare, "",
							"", bean.fstRpNextCreatedTime,
							bean.fstRepLastCreatedTime,
							bean.fstRepInitialCreatedTime, bean.fstRepDateOne,
							bean.fstRepDateTwo, bean.fstRecommendName, url,
