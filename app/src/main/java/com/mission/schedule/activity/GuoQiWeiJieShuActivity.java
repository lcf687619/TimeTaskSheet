...
import com.mission.schedule.adapter.GuoQiWeiJieShuAdapter;
...
public class GuoQiWeiJieShuActivity extends BaseActivity implements
		OnClickListener, OnItemClickListener {

	@ViewResId(id = R.id.top_ll_back)
	private LinearLayout top_ll_back;
	@ViewResId(id = R.id.myguoqiweijieshu_listview)
	private SwipeXListViewNoHead myguoqiweijieshu_listview;
	@ViewResId(id = R.id.piliang_tv)
	private TextView piliang_tv;

	private List<Map<String, String>> mList = new ArrayList<Map<String, String>>();
	private List<Map<String, String>> yestodaylist = new ArrayList<Map<String, String>>();
	private List<Map<String, String>> otherlist = new ArrayList<Map<String, String>>();
	GuoQiWeiJieShuAdapter adapter = null;
	Context context;
	...
	protected void setListener() {
		...
		myguoqiweijieshu_listview.setPullLoadEnable(false);
		piliang_tv.setOnClickListener(this);
	}

	@Override
	protected void setContentView() {
		setContentView(R.layout.activity_guoqiweijieshu);
	}
	...
	protected void init(Bundle savedInstanceState) {
		...
		myguoqiweijieshu_listview.addFooterView(footView);
		loadData();
	}
	...
	private void loadData() {
		...
		mList.clear();
		try {
			yestodaylist = application.queryAllSchData(17,0, 0);

			otherlist = application.queryAllSchData(18,0, 0);
	...
	protected void setAdapter() {
		adapter = new GuoQiWeiJieShuAdapter(context, mList,
				R.layout.adapter_guoqiweijieshu, handler,
				myguoqiweijieshu_listview, mScreenWidth);
		myguoqiweijieshu_listview.setPullLoadEnable(false);
		myguoqiweijieshu_listview.setPullRefreshEnable(false);
		myguoqiweijieshu_listview.setAdapter(adapter);
	}
	...
	public void onClick(View v) {
		...
		case R.id.top_ll_back:
			this.finish();
			break;
		case R.id.piliang_tv:
			if (mList.size() > 0) {
				setIsEndDialog(0);
			} else {
				Toast.makeText(context, "没有需要结束的记事!", Toast.LENGTH_LONG).show();
				return;
			}
			break;
		default:
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 2:// 设为结束
				...
				myguoqiweijieshu_listview.invalidate();
			...
			public void onClick(View v) {
				...
				if (type == 0) {
					try {
						...
						App.getDBcApplication()
								.updateSchRepeatLinkData(
										Integer.parseInt(mMap
												.get(ScheduleTable.schID)), 2);
						...
						myguoqiweijieshu_listview.hiddenRight();
	...
	private void setIsEndDialog(final int type) {
		final AlertDialog builder = new AlertDialog.Builder(context).create();
		builder.show();
		Window window = builder.getWindow();
		android.view.WindowManager.LayoutParams params = window.getAttributes();// 获取LayoutParams
		...
		delete_ok.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				builder.cancel();
				if (type == 0) {
					application.updateGuoQiWeiJieShu();
					for (Map<String, String> map : mList) {
						if ("0".equals(map.get(ScheduleTable.schRepeatID))) {
							...
							updateScheduleRead2(map, ScheduleTable.schRead,
									ScheduleTable.schRepeatLink);
						}
						updateFocusStateSch(map, ScheduleTable.schFocusState);
						updateScheduleIsEnd(map, ScheduleTable.schIsEnd,
								ScheduleTable.schUpdateState);
					}
					loadData();
					adapter.notifyDataSetChanged();
	...
	class MyGeneralDetailOnClick implements View.OnClickListener {
		...
		public MyGeneralDetailOnClick(Dialog dialog, Map<String, String> mMap,
			...
			detail_date.setText(CharacterUtil.getWeekOfDate(context,
		...
		public void onClick(View v) {
			...
			case R.id.more_setdaiban:// 设为待办 今天+全天+顺延
				try {
					...
					adapter.notifyDataSetChanged();
					// myguoqiweijieshu_listview.setSelectionFromTop(scrolledX,
					// scrolledY);
					RefreshMyData();
				} catch (Exception e) {
					e.printStackTrace();
			...
			case R.id.more_shitingmusic:// 试听铃声
				...
				try {
					AssetFileDescriptor fileDescriptor = context.getAssets()
							.openFd(mMap.get(ScheduleTable.schRingCode)
									+ ".mp3");
	...
	private void deleteDialog(final String id, final Map<String, String> mMap,
			final int type) {
		final AlertDialog builder = new AlertDialog.Builder(context).create();
		builder.show();
		Window window = builder.getWindow();
		...
		delete_ok.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (type == 0) {
					String deleteId = id;
					App dbContextExtended = App.getDBcApplication();
					dbContextExtended.deleteScheduleData(Integer
							.parseInt(deleteId));
					mList.clear();
					loadData();
					adapter.notifyDataSetChanged();
