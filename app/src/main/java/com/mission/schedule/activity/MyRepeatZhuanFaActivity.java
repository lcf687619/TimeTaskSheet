...
public class MyRepeatZhuanFaActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void init(Bundle savedInstanceState) {
		...
		name = prefUtil.getString(context, ShareFile.USERFILE,
				ShareFile.USERNAME, "");
		bean = (RepeatBean) getIntent().getSerializableExtra("bean");
		LoadData();
	}
		...
		friends_lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				boolean fag = app.insertMessageSendData(
						...
						Integer.parseInt(bean.repOpenState), 2,
						Integer.parseInt(bean.repIsAlarm), 0, 0,
						Integer.parseInt(bean.repDisplayTime),
						Integer.parseInt(bean.repBeforeTime),
						Integer.parseInt(bean.repSourceType),
						Integer.parseInt(bean.repType), 0, 0, 0, 0, 0,
						bean.repTypeParameter, bean.repContent,
						bean.repNextCreatedTime.substring(0, 10), bean.repTime,
						bean.repSourceDesc, bean.repSourceDescSpare, "",
						bean.repRingDesc, bean.repRingCode, bean.repStartDate,
						bean.repInitialCreatedTime, bean.repLastCreatedTime,
						bean.repNextCreatedTime, bean.webUrl, bean.imgPath,
						name, "", 1, 0, 0, "", 0, 0, "", 0, bean.parReamrk,
						DateUtil.formatDateTimeSs(new Date()));
				ID = app.nfmId;
				if (fag) {
					app.insertMessageSendData(Integer.parseInt(userId),
							friendsBean.fId, 0,
							Integer.parseInt(bean.repOpenState), 1,
							Integer.parseInt(bean.repIsAlarm), 0, 0,
							Integer.parseInt(bean.repDisplayTime),
							Integer.parseInt(bean.repBeforeTime),
							Integer.parseInt(bean.repSourceType), 0,
							bean.aType, 0, 0, 0, 0, "", bean.repContent,
							bean.repNextCreatedTime.substring(0, 10),
							bean.repTime, bean.repSourceDesc,
							bean.repSourceDescSpare, "", bean.repRingDesc,
							bean.repRingCode, bean.repStartDate,
							bean.repInitialCreatedTime,
							bean.repLastCreatedTime, bean.repNextCreatedTime,
							bean.webUrl, bean.imgPath, name, "", 0, ID, 0, "",
							0, 0, "", 0, bean.parReamrk,
							DateUtil.formatDateTimeSs(new Date()));
				}
				UpdateLoadData();
			}
