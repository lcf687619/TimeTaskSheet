...
public class EditSendMessageActivity extends BaseActivity implements
		OnClickListener {
	...
	int isAlarm = 1; // 共4种：0 无闹钟 | 1 准时有闹钟 提前无闹钟 | 2 准时无闹钟 提前有闹钟 | 3 准时提前均有闹钟
	...
	public void onClick(View v) {
		...
		case R.id.top_ll_send:
			try {
				...
				app.insertMessageSendData(Integer.parseInt(userid), friendid,
						0, 0, 1, isAlarm, Integer.parseInt(postpone), 0,
						timestate, before, 0, 0, atype, 0, 0, 0, 0, "",
						message, date, DateUtil.formatDateTimeHm(DateUtil
								.parseDateTimeHm(time)), "", "", "",
						alamsoundDesc, alamsound, "", "", "", "", url, "",
						userName, "", 1, 0, 0, "", 0, 0, "", 0, "", DateUtil
		...
		public View getView(final int position, View convertView,
			...
			if (select == 1) {
				...
				if (map.get(0)) {
					...
					HHmmtime_tv.setText(time_tv.getText().toString());
				} else {
					if (map.get(position)) {
						isAlarm = 2;
