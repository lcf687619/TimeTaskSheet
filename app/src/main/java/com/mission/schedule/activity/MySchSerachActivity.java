...
import com.mission.schedule.adapter.MySchSerachAdapter;
...
public class MySchSerachActivity extends BaseActivity implements
		OnClickListener {
	...
	@ViewResId(id = R.id.tishi_tv)
	private TextView tishi_tv;
	@ViewResId(id = R.id.myschedule_listview)
	private SwipeXListViewNoHead myschedule_listview;
	...
	MySchSerachAdapter adapter = null;
	String mContent = "";
	// int scrolledX = 0;
	// int scrolledY = 0;
	...
	protected void setContentView() {
		setContentView(R.layout.activity_myschserach);
	}

	@Override
	protected void init(Bundle savedInstanceState) {
		context = this;
		headtitle_tv.setText("日程搜索");
		myschedule_listview.setPullLoadEnable(false);
		myschedule_listview.setPullRefreshEnable(false);
		...
		myschedule_listview.addFooterView(footView);
		mContent = getIntent().getStringExtra("content");
		sousuocontent_et.setText(mContent);
		if (!"".equals(mContent)) {
			sousuocontent_et.setSelection(mContent.length());
			loadData(mContent);
		} else {
			tishi_tv.setVisibility(View.GONE);
		}
		// myschedule_listview.setOnScrollListener(new OnScrollListener() {
		//
		// /**
		// * 滚动状态改变时调用
		// */
		...
		// scrolledX = myschedule_listview.getFirstVisiblePosition();
		// }
		// if (mList != null) {
		// View v = myschedule_listview.getChildAt(0);
	...
	private void loadData(String content) {
		...
		otherlist = application.QuerySchSerachData(1, content);
		yestodaylist = application.QuerySchSerachData(2, content);
		todaylist = application.QuerySchSerachData(3, content);
		tomorrowlist = application.QuerySchSerachData(4, content);
		inweeklist = application.QuerySchSerachData(5, content);
		outweeklist = application.QuerySchSerachData(6, content);
		...
		adapter = new MySchSerachAdapter(context, mList, R.layout.adapter_important, handler,
				myschedule_listview, mScreenWidth);
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.sousuo_iv:
			loadData(sousuocontent_et.getText().toString());
			break;
		...
		public MyGeneralDetailOnClick(Dialog dialog, Map<String, String> mMap,
			...
			try {
				...
				if (today.equals(key)) {// 今天
					if (DateUtil.parseDateTime(
							DateUtil.formatDateTime(new Date())).after(
				...
				if ("0".equals(mMap.get(ScheduleTable.schIsImportant))) {
					...
					detail_important.setCompoundDrawables(null, jieshu, null,
							null);
				} else {
					detail_important.setText("取消重要");
					detail_important.setCompoundDrawables(null, weijieshu,
							null, null);
				}
